...
function SortUI()
{
	...
	for( let key in m_ProfileItem['filament'] )
	{
		...
		let FilamentItem={key: key, data: OneFila};
		if(IsUniversal && IsGeneric)
			UniversalGenericFilamentArray.push(FilamentItem);
		else if(IsUniversal)
			UniversalFilamentArray.push(FilamentItem);
		else if(IsGeneric)
			ModelGenericFilamentArray.push(FilamentItem);
		else
			ModelFilamentArray.push(FilamentItem);
	}
...
function ChooseDefaultFilament()
{
	...
	{
		let OneMode=m_ProfileItem["model"][n];
		if(ModelList.indexOf(OneMode['model'])>-1)
		{
    ...
    for(let m=0;m<nFilament;m++)
	{
		...
		let filamentNames=SplitFilamentList(OneFF.getAttribute("filalist"));
		let IsDefault=false;
		for(let n=0;n<filamentNames.length;n++)
		{
			if(DefaultMaterialNames[filamentNames[n]]===true)
			{
				IsDefault=true;
				break;
			}
