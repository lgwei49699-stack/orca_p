...
public:
    ...
    void                        load_config_model(const std::string &name, DynamicPrintConfig config, Semver file_version = Semver(), bool merge_system_differences = true)
        { this->load_config_file_config(name, true, std::move(config), file_version, false, false, merge_system_differences); }
...
private:
    ...
    void                        load_config_file_config(const std::string &name_or_path, bool is_external, DynamicPrintConfig &&config, Semver file_version = Semver(), bool selected = false, bool is_custom_defined = false, bool merge_system_differences = true);
