...
public:
    ...
    bool                        has_raft()              const
    {
        // CuraV1 is itself the raft enable switch. Its mandatory phase counts are
        // validated by Print::validate(); do not reinterpret an invalid external
        // configuration as an unrafted object before validation can report it.
        return m_config.raft_mode.value == RaftMode::CuraV1 || m_config.raft_layers.value > 0;
    }
...
public:
    ...
    std::string         export_gcode(const std::string& path_template, GCodeProcessorResult* result,
                                     ...
                                     bool force_toolchange_for_single_extruder = false,
                                     bool embed_thumbnail_image = true);
