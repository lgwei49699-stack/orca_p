...
    AutoArranger<BinT> arranger_first_try{corrected_bin, mod_params, progressfn, stopfn};
...
    for (const auto& bin_items : result_first_try) {
        packed_items_count_first_try += bin_items.size();
    }
    
    if (packed_items_count_first_try == items_to_arrange.size()) {
        arrange_success = true;
        BOOST_LOG_TRIVIAL(error) << "CALL_SITE_LOG: _arrange: First arrangement attempt SUCCEEDED. All " << packed_items_count_first_try << " items packed.";
    } else {
        BOOST_LOG_TRIVIAL(error) << "CALL_SITE_LOG: _arrange: First arrangement attempt FAILED. Packed " << packed_items_count_first_try << " of " << items_to_arrange.size() << " items.";
    }


    if (!arrange_success && params.allow_rotations) {
        BOOST_LOG_TRIVIAL(error) << "CALL_SITE_LOG: _arrange: Arrangement failed with initial rotations. Retrying with 0-degree rotation.";
        
        // 重置所有模型的旋转角度为0
        for (auto& item : items_to_arrange) {
            item.rotation(0);
            BOOST_LOG_TRIVIAL(error) << "CALL_SITE_LOG: _arrange: Item " << item.name << " rotation reset to 0.";
        }

        // 再次调用排列算法
        AutoArranger<BinT> arranger_second_try{corrected_bin, mod_params, progressfn, stopfn};
        remove_large_items(excludes, corrected_bin); // 重新处理排除项
...
        if (packed_items_count_second_try == items_to_arrange.size()) {
            arrange_success = true;
