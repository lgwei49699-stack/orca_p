...
#include <boost/thread.hpp>
    ...
    // Structured CLI result reporting is shared by Linux, macOS and Windows.
...
std::map<int, std::string> cli_errors =
    {{CLI_SUCCESS, "Success."},
     ...
     {CLI_MODEL_REPAIR_ERROR, "Failed repairing the model."},
     {CLI_RESULT_WRITE_ERROR, "Failed writing the CLI result file."},
     {CLI_NO_SUITABLE_OBJECTS, "One of the plate is empty or has no object fully inside it. Please check that the 3mf contains no empty "
                               "plate in Orca Slicer before uploading."},
     {CLI_VALIDATE_ERROR,
     ...
     {CLI_SLICING_TIME_EXCEEDS_LIMIT,
      "Slicing time of a certain plate exceeds the limit. Please simplify the model or use a larger slicing layer height."},
     {CLI_TRIANGLE_COUNT_EXCEEDS_LIMIT,
      "Triangle count of single plate exceeds the limit. Please simplify the model and try to upload again."},
     {CLI_NO_SUITABLE_OBJECTS_AFTER_SKIP, "No printable objects to slice after skipping."},
     {CLI_FILAMENT_NOT_MATCH_BED_TYPE,
      "Filaments are not compatible with the plate type. Please verify the slicing of all plates in Orca Slicer before uploading."},
     {CLI_FILAMENTS_DIFFERENT_TEMP, "The temperature difference of the filaments used is too large. Please verify the slicing of all "
                                    "plates in Orca Slicer before uploading."},
     {CLI_OBJECT_COLLISION_IN_SEQ_PRINT, "Object conflicts were detected when using print-by-object mode. Please verify the slicing of all "
                                         "plates in Orca Slicer before uploading."},
     ...
     {CLI_SPIRAL_MODE_INVALID_PARAMS,
      "Some slicing parameters cannot work with Spiral Vase mode. Please solve the issue in Orca Slicer before uploading."},
     ...
     {CLI_GCODE_PATH_CONFLICTS,
      " G-code conflicts detected after slicing. Please make sure the 3mf file can be successfully sliced in the latest Orca Slicer."}};
...
typedef struct _model_repair_info {
    ...
    std::string              engine{"admesh"};
    MeshRepairReport         report;
    ModelRepairReport        cgal_report;
    std::vector<std::string> output_files;
} model_repair_info_t;

typedef struct _obj_export_info
{
    std::string              mode;
    std::string              semantic_source;
    bool                     original_mtl_preserved{false};
    bool                     uv_preserved{false};
    bool                     texture_preserved{false};
    std::vector<std::string> output_files;
} obj_export_info_t;
...
typedef struct _sliced_info {
    ...
    std::vector<obj_export_info_t>   obj_exports;
    json                             model_transforms = nlohmann::json::array();
}sliced_info_t;
...
static bool is_replaceable_cli_result(const boost::filesystem::path& result_path)
{
    boost::system::error_code            status_error;
    const boost::filesystem::file_status status = boost::filesystem::symlink_status(result_path, status_error);
    if (status.type() == boost::filesystem::file_not_found)
        return true;
    if (status_error || boost::filesystem::is_symlink(status) || !boost::filesystem::is_regular_file(status)) {
        BOOST_LOG_TRIVIAL(error) << "Refusing to replace non-regular CLI result file " << result_path.string();
        boost::nowide::cerr << "Refusing to replace non-regular CLI result file " << result_path.string() << std::endl;
        return false;
    }
    return true;
}

static void remove_cli_result_file(const boost::filesystem::path& path)
{
    boost::system::error_code ignored;
    boost::filesystem::remove(path, ignored);
}

static bool write_cli_result_atomically(const boost::filesystem::path& result_path, const std::string& contents)
{
    if (!is_replaceable_cli_result(result_path))
        return false;

    boost::filesystem::path temporary;
    boost::filesystem::path backup;
    try {
        const boost::filesystem::path directory = result_path.parent_path();
        temporary = directory / boost::filesystem::unique_path(result_path.filename().string() + ".tmp-%%%%-%%%%");
        backup    = directory / boost::filesystem::unique_path(result_path.filename().string() + ".bak-%%%%-%%%%");
    } catch (const boost::filesystem::filesystem_error& exception) {
        BOOST_LOG_TRIVIAL(error) << "Failed creating temporary CLI result paths: " << exception.what();
        return false;
    }

    boost::nowide::ofstream result_stream(temporary.string(), std::ios::out | std::ios::trunc);
    if (!result_stream.is_open()) {
        BOOST_LOG_TRIVIAL(error) << "Failed opening temporary CLI result file " << temporary.string();
        boost::nowide::cerr << "Failed opening CLI result file " << result_path.string() << std::endl;
        return false;
    }
    result_stream << contents;
    result_stream.flush();
    bool write_succeeded = result_stream.good();
    result_stream.close();
    write_succeeded = write_succeeded && !result_stream.fail();
    if (!write_succeeded) {
        remove_cli_result_file(temporary);
        BOOST_LOG_TRIVIAL(error) << "Failed writing temporary CLI result file " << temporary.string();
        boost::nowide::cerr << "Failed writing CLI result file " << result_path.string() << std::endl;
        return false;
    }

    // Re-check after serialization so a substituted symlink or directory is
    // never followed during installation.
    if (!is_replaceable_cli_result(result_path)) {
        remove_cli_result_file(temporary);
        return false;
    }
    ...
    if (status_error) {
        remove_cli_result_file(temporary);
        BOOST_LOG_TRIVIAL(error) << "Failed inspecting CLI result file " << result_path.string() << ": " << status_error.message();
        return false;
    }
    ...
    if (status.type() != boost::filesystem::file_not_found) {
        ...
        boost::filesystem::rename(result_path, backup, backup_error);
        if (backup_error) {
            remove_cli_result_file(temporary);
            BOOST_LOG_TRIVIAL(error) << "Failed backing up CLI result file " << result_path.string() << ": " << backup_error.message();
            return false;
        }
        backed_up = true;
    }
    ...
    boost::filesystem::rename(temporary, result_path, install_error);
    if (install_error) {
        remove_cli_result_file(temporary);
        if (backed_up) {
            boost::system::error_code restore_error;
            boost::filesystem::rename(backup, result_path, restore_error);
            if (restore_error)
                BOOST_LOG_TRIVIAL(error) << "Failed restoring CLI result file " << result_path.string() << "; the original remains at "
                                         << backup.string() << ": " << restore_error.message();
        }
        BOOST_LOG_TRIVIAL(error) << "Failed installing CLI result file " << result_path.string() << ": " << install_error.message();
        return false;
    }

    if (backed_up) {
        boost::system::error_code remove_error;
        boost::filesystem::remove(backup, remove_error);
        if (remove_error)
            BOOST_LOG_TRIVIAL(warning) << "Failed removing obsolete CLI result backup " << backup.string() << ": "
                                       << remove_error.message();
    }
    return true;
}

bool record_exit_reson(std::string                        outputdir,
                       int                                code,
                       int                                plate_id,
                       std::string                        error_message,
                       sliced_info_t&                     sliced_info,
                       std::map<std::string, std::string> key_values = std::map<std::string, std::string>())
{
    boost::filesystem::path result_path("result.json");
    if (!outputdir.empty()) {
        boost::system::error_code error_code;
        boost::filesystem::create_directories(outputdir, error_code);
        if (error_code) {
            BOOST_LOG_TRIVIAL(error) << "Failed creating CLI result directory " << outputdir << ": " << error_code.message();
            boost::nowide::cerr << "Failed creating CLI result directory " << outputdir << ": " << error_code.message() << std::endl;
            return false;
        }
        result_path = boost::filesystem::path(outputdir) / "result.json";
    }
    const std::string result_file = result_path.string();
    ...
    try {
        ...
        j["output_files"] = sliced_info.output_files;
        if (!sliced_info.model_transforms.empty())
            j["model_transforms"] = sliced_info.model_transforms;
        j["obj_exports"] = json::array();
        for (const obj_export_info_t& obj_export : sliced_info.obj_exports) {
            j["obj_exports"].push_back({{"mode", obj_export.mode},
                                        {"semantic_source", obj_export.semantic_source},
                                        {"original_mtl_preserved", obj_export.original_mtl_preserved},
                                        {"uv_preserved", obj_export.uv_preserved},
                                        {"texture_preserved", obj_export.texture_preserved},
                                        {"output_files", obj_export.output_files}});
        }
        double total_time = 0.0;
        double total_filament_g = 0.0;
        double support_filament_g = 0.0;
        ...
        for (const model_repair_info_t &repair_info : sliced_info.model_repairs) {
            ...
            repair_json["engine"]       = repair_info.engine;

            if (repair_info.engine == "cgal") {
                const ModelRepairReport& report = repair_info.cgal_report;
                repair_json["repair_enabled"]   = true;
                repair_json["repair_attempted"] = report.attempted;
                repair_json["load_succeeded"]   = true;
                repair_json["status"]           = report.status();
                repair_json["success"]          = report.success;
                repair_json["committed"]        = report.committed;
                repair_json["before"]           = {{"facets", report.before_facets}, {"open_edges", report.before_open_edges}};
                repair_json["repair_actions"]   = {{"objects", report.objects},
                                                   {"volumes", report.volumes},
                                                   {"repaired", report.repaired},
                                                   {"mesh_repaired", report.mesh_repaired},
                                                   {"skipped", report.skipped},
                                                   {"failed", report.failed},
                                                   {"rolled_back", report.rolled_back},
                                                   {"split_volumes", report.split_volumes},
                                                   {"created_parts", report.created_parts},
                                                   {"removed_parts", report.removed_parts}};
                repair_json["after"]            = {{"facets", report.after_facets},
                                                   {"open_edges", report.after_open_edges},
                                                   {"closed", report.success && report.after_facets > 0 && report.after_open_edges == 0}};
                repair_json["volumes"]          = json::array();
                repair_json["failures"]         = json::array();
                for (const ModelRepairVolumeReport& volume : report.volume_reports) {
                    repair_json["volumes"].push_back(
                        {{"object_index", volume.object_index},
                         {"volume_index", volume.volume_index},
                         {"object_name", volume.object_name},
                         {"volume_name", volume.volume_name},
                         {"status", volume.status()},
                         {"message", volume.message},
                         {"annotations_cleared", volume.annotations_cleared},
                         {"annotations_preserved", volume.annotations_preserved},
                         {"repaired_parts", volume.repaired_parts},
                         {"split_parts", volume.split_parts},
                         {"removed_parts", volume.removed_parts},
                         {"before", {{"facets", volume.before_facets}, {"open_edges", volume.before_open_edges}}},
                         {"after", {{"facets", volume.after_facets}, {"open_edges", volume.after_open_edges}}}});
                }
                for (const ModelRepairFailure& failure : report.failures) {
                    repair_json["failures"].push_back({{"object_index", failure.object_index},
                                                       {"volume_index", failure.volume_index},
                                                       {"object_name", failure.object_name},
                                                       {"volume_name", failure.volume_name},
                ...
                repair_json["status"]           = report.status();
                repair_json["before"]           = {{"facets", report.original_facets}, {"open_edges", report.initial_open_edges}};
                repair_json["repair_actions"]   = {{"edges_fixed", report.edges_fixed},
                ...
                repair_json["after"]            = {{"facets", report.final_facets},
                                                   {"open_edges", report.remaining_open_edges},
                                                   {"backwards_edges", report.remaining_backwards_edges},
                                                   {"manifold", report.load_succeeded && report.remaining_open_edges == 0 &&
                                                                    report.remaining_backwards_edges == 0}};
            }
        ...
        const std::string result_contents = j.dump(4) + "\n";
        if (!write_cli_result_atomically(result_path, result_contents))
            return false;

        BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" <<__LINE__ << boost::format(", saved config to %1%\n")%result_file;
        return true;
    } catch (const std::exception& exception) {
        BOOST_LOG_TRIVIAL(error) << "Failed serializing CLI result file " << result_file << ": " << exception.what();
    } catch (...) {
        BOOST_LOG_TRIVIAL(error) << "Failed serializing CLI result file " << result_file << " with an unknown error";
    }
    return false;
}

static void attach_exported_model_paths(sliced_info_t&                               sliced_info,
                                        const std::vector<std::vector<std::string>>& exported_paths,
                                        size_t                                       source_model_count)
{
    for (const std::vector<std::string>& model_paths : exported_paths) {
        for (const std::string& path : model_paths) {
            if (std::find(sliced_info.output_files.begin(), sliced_info.output_files.end(), path) == sliced_info.output_files.end())
                sliced_info.output_files.push_back(path);
        }
    }

    for (model_repair_info_t &repair_info : sliced_info.model_repairs) {
        const std::vector<std::string>* paths = nullptr;
        // Multiple repaired inputs are merged into one Model before export. In
        // that case the shared artifact truthfully belongs to every source
        // repair report, while the top-level list remains de-duplicated.
        if (exported_paths.size() == 1 && source_model_count > 0 && sliced_info.model_repairs.size() == source_model_count)
            paths = &exported_paths.front();
        else if (exported_paths.size() == sliced_info.model_repairs.size() && source_model_count == exported_paths.size() &&
                 repair_info.model_index < exported_paths.size())
            paths = &exported_paths[repair_info.model_index];
        if (paths == nullptr)
            continue;
        for (const std::string& path : *paths) {
            if (std::find(repair_info.output_files.begin(), repair_info.output_files.end(), path) == repair_info.output_files.end())
                repair_info.output_files.push_back(path);
        }
    }
}
...
int CLI::run(int argc, char **argv)
{
    ...
    const bool  cgal_repair_requested = std::find(m_actions.begin(), m_actions.end(), "repair_model") != m_actions.end();
    std::string repair_engine;
    if (cgal_repair_requested) {
        repair_engine = boost::algorithm::to_lower_copy(m_config.opt_string("repair_model"));
        if (repair_engine != "cgal") {
            const std::string error_message = "Unsupported --repair-model engine '" + repair_engine + "'; expected 'cgal'.";
            boost::nowide::cerr << error_message << std::endl;
            record_exit_reson(outfile_dir, CLI_INVALID_PARAMS, 0, error_message, sliced_info);
            flush_and_exit(CLI_INVALID_PARAMS);
        }
        if (m_input_files.empty()) {
            const std::string error_message = "--repair-model cgal currently requires at least one STL or OBJ input file.";
            boost::nowide::cerr << error_message << std::endl;
            record_exit_reson(outfile_dir, CLI_INVALID_PARAMS, 0, error_message, sliced_info);
            flush_and_exit(CLI_INVALID_PARAMS);
        }
        for (const std::string& input_file : m_input_files) {
            if (!boost::algorithm::iends_with(input_file, ".stl") && !boost::algorithm::iends_with(input_file, ".obj")) {
                const std::string error_message = "--repair-model cgal currently supports STL and OBJ inputs; OBJ repair operates on "
                                                  "geometry only and does not preserve material semantics. Unsupported input: " +
                                                  input_file;
                boost::nowide::cerr << error_message << std::endl;
                record_exit_reson(outfile_dir, CLI_INVALID_PARAMS, 0, error_message, sliced_info);
                flush_and_exit(CLI_INVALID_PARAMS);
        ...
        if (auto_repair_model) {
            BOOST_LOG_TRIVIAL(info) << "--repair-model cgal replaces the legacy STL load-time repair for this run";
            auto_repair_model = false;
        }
    }
    ...
    if (cgal_repair_requested && export_texture_obj_requested) {
        const std::string error_message = "--repair-model cgal cannot be combined with --export-texture-obj because repair may change "
                                          "topology; use --export-obj for geometry-only OBJ output.";
        boost::nowide::cerr << error_message << std::endl;
        record_exit_reson(outfile_dir, CLI_INVALID_PARAMS, 0, error_message, sliced_info);
        flush_and_exit(CLI_INVALID_PARAMS);
    }
    ...
    std::vector<std::string> loaded_model_sources;
    ...
    if (load_assemble_list.empty()) {
        for (const std::string& file : m_input_files) {
            ...
            Model model;
            const bool collect_repair_report = !cgal_repair_requested &&
                                               (boost::algorithm::iends_with(file, ".stl") || boost::algorithm::iends_with(file, ".oltp"));
            ...
            loaded_model_sources.push_back(file);
        ...
        m_models.push_back(std::move(model));
        loaded_model_sources.emplace_back();
    }
    ...
    // Repair each imported model before model merging and before any export,
    // so reports retain their source-file identity and all outputs see the
    // same committed geometry.
    if (cgal_repair_requested) {
        for (size_t model_index = 0; model_index < m_models.size(); ++model_index) {
            model_repair_info_t repair_info;
            repair_info.engine      = repair_engine;
            repair_info.model_index = model_index;
            if (model_index < loaded_model_sources.size())
                repair_info.input_file = loaded_model_sources[model_index];
            else if (!m_models[model_index].objects.empty())
                repair_info.input_file = m_models[model_index].objects.front()->input_file;

            const bool               repair_succeeded = repair_model(m_models[model_index], repair_info.cgal_report);
            const ModelRepairReport& report           = repair_info.cgal_report;
            BOOST_LOG_TRIVIAL(info)
                << boost::format("CGAL model repair summary: input=%1%, status=%2%, committed=%3%, volumes=%4%, repaired=%5%, "
                                 "mesh_repaired=%6%, split=%7%, removed_parts=%8%, skipped=%9%, facets=%10%->%11%, open_edges=%12%->%13%") %
                       repair_info.input_file % report.status() % report.committed % report.volumes % report.repaired %
                       report.mesh_repaired % report.split_volumes % report.removed_parts % report.skipped % report.before_facets %
                       report.after_facets % report.before_open_edges % report.after_open_edges;

            std::string repair_error = cli_errors[CLI_MODEL_REPAIR_ERROR];
            if (!report.failures.empty()) {
                const ModelRepairFailure& failure = report.failures.front();
                repair_error += " Object '" + failure.object_name + "', volume '" + failure.volume_name + "': " + failure.message;
            }
            sliced_info.model_repairs.push_back(std::move(repair_info));

            if (!repair_succeeded) {
                BOOST_LOG_TRIVIAL(error) << repair_error;
                record_exit_reson(outfile_dir, CLI_MODEL_REPAIR_ERROR, 0, repair_error, sliced_info);
                flush_and_exit(CLI_MODEL_REPAIR_ERROR);
            }
        }
    }
    ...
    if (export_transforms) {
        ...
        sliced_info.model_transforms = std::move(model_transforms_json);
    }
    ...
    // Resolve every OBJ-family artifact before the first exporter runs. This
    // prevents actions from overwriting each other and protects source OBJ/MTL
    // files recorded inside containers such as 3MF, not only positional input.
    std::vector<boost::filesystem::path> protected_obj_sources;
    const auto                           append_protected_obj_source = [&protected_obj_sources](const std::string& source) {
        if (source.empty())
            return;
        boost::filesystem::path source_path(source);
        protected_obj_sources.push_back(source_path);
        if (boost::algorithm::iends_with(source_path.extension().string(), ".obj")) {
            boost::filesystem::path conventional_mtl = source_path;
            conventional_mtl.replace_extension(".mtl");
            protected_obj_sources.push_back(std::move(conventional_mtl));

            boost::system::error_code source_error;
            if (boost::filesystem::is_regular_file(source_path, source_error) && !source_error) {
                std::vector<std::string> material_libraries;
                std::string              parse_message;
                if (load_obj_material_libraries(source_path.string().c_str(), material_libraries, parse_message))
                    for (const std::string& library : material_libraries)
                        protected_obj_sources.emplace_back(library);
            }
        }
    };
    for (const std::string& input_file : m_input_files)
        append_protected_obj_source(input_file);
    for (const Model& model : m_models) {
        for (const ModelObject* object : model.objects) {
            if (object == nullptr)
                continue;
            append_protected_obj_source(object->input_file);
            for (const ModelVolume* volume : object->volumes)
                if (volume != nullptr)
                    append_protected_obj_source(volume->source.input_file);
        }
    }

    const auto resolved_obj_path = [](const boost::filesystem::path& path, boost::system::error_code& error) {
        const boost::filesystem::path absolute = boost::filesystem::absolute(path, error);
        if (error)
            return boost::filesystem::path{};
        return boost::filesystem::weakly_canonical(absolute, error);
    };
    const auto obj_path_key = [&resolved_obj_path](const boost::filesystem::path& path, boost::system::error_code& error) {
        std::string key = resolved_obj_path(path, error).generic_string();
#if defined(_WIN32) || defined(__APPLE__)
        boost::algorithm::to_lower(key);
#endif
        return key;
    };
    std::set<std::string> planned_obj_artifacts;
    const auto claim_obj_artifact = [&](const boost::filesystem::path& path, const std::string& action, std::string& error_message) {
        boost::system::error_code path_error;
        const std::string         key = obj_path_key(path, path_error);
        if (path.empty() || path_error) {
            error_message = "Could not resolve " + action + " output path.";
            return false;
        }

        const boost::filesystem::path absolute_output = resolved_obj_path(path, path_error);
        if (path_error) {
            error_message = "Could not resolve " + action + " output path.";
            return false;
        }
        for (const boost::filesystem::path& source : protected_obj_sources) {
            boost::system::error_code     source_error;
            const boost::filesystem::path absolute_source = resolved_obj_path(source, source_error);
            if (source_error)
                continue;
#if defined(_WIN32) || defined(__APPLE__)
            const bool same_path = boost::algorithm::iequals(absolute_output.generic_string(), absolute_source.generic_string());
#else
            const bool same_path = absolute_output == absolute_source;
#endif
            boost::system::error_code equivalent_error;
            const bool equivalent = boost::filesystem::exists(absolute_output) && boost::filesystem::exists(absolute_source) &&
                                    boost::filesystem::equivalent(absolute_output, absolute_source, equivalent_error) && !equivalent_error;
            if (same_path || equivalent) {
                error_message = action + " output must not overwrite a source OBJ or its MTL sidecar: " + path.string();
                return false;
            }
        }
        if (!planned_obj_artifacts.insert(key).second) {
            error_message = "OBJ export actions resolve to the same output artifact: " + path.string();
            return false;
        }
        return true;
    };
    const auto explicit_obj_path = [&outfile_dir, this](const std::string& option) {
        boost::filesystem::path path(m_config.opt_string(option));
        if (path.is_relative() && !outfile_dir.empty())
            path = boost::filesystem::path(outfile_dir) / path;
        if (!boost::algorithm::iends_with(path.extension().string(), ".obj"))
            path.replace_extension(".obj");
        return path;
    };

    std::string obj_preflight_error;
    for (const std::string& action : m_actions) {
        if (action == "export_multicolor_obj" || action == "export_texture_obj") {
            boost::filesystem::path obj_path = explicit_obj_path(action);
            boost::filesystem::path mtl_path = obj_path;
            mtl_path.replace_extension(".mtl");
            if (!claim_obj_artifact(obj_path, "--" + boost::algorithm::replace_all_copy(action, "_", "-"), obj_preflight_error) ||
                !claim_obj_artifact(mtl_path, "--" + boost::algorithm::replace_all_copy(action, "_", "-"), obj_preflight_error))
                break;
        } else if (action == "export_obj" && m_config.opt_bool(action)) {
            for (const Model& model : m_models) {
                const boost::filesystem::path obj_path(this->output_filepath(model, IO::OBJ));
                if (!claim_obj_artifact(obj_path, "--export-obj", obj_preflight_error))
                    break;
            }
            if (!obj_preflight_error.empty())
                break;
        }
    }
    if (!obj_preflight_error.empty()) {
        BOOST_LOG_TRIVIAL(error) << obj_preflight_error;
        record_exit_reson(outfile_dir, CLI_EXPORT_OBJ_ERROR, 0, obj_preflight_error, sliced_info);
        flush_and_exit(CLI_EXPORT_OBJ_ERROR);
    }

    const auto object_has_exportable_geometry = [](const ModelObject* object) {
        return object != nullptr && std::any_of(object->volumes.begin(), object->volumes.end(), [](const ModelVolume* volume) {
                   return volume != nullptr && volume->is_model_part() && !volume->mesh().empty();
               });
    };
    const auto model_has_exportable_geometry = [&object_has_exportable_geometry](const Model& model) {
        return std::any_of(model.objects.begin(), model.objects.end(), object_has_exportable_geometry);
    };
    const auto all_stl_objects_have_exportable_geometry = [&object_has_exportable_geometry, this]() {
        return !m_models.empty() && std::all_of(m_models.begin(), m_models.end(), [&object_has_exportable_geometry](const Model& model) {
            return !model.objects.empty() && std::all_of(model.objects.begin(), model.objects.end(), object_has_exportable_geometry);
        });
    };
    const auto all_obj_models_have_exportable_geometry = [&model_has_exportable_geometry, this]() {
        return !m_models.empty() && std::all_of(m_models.begin(), m_models.end(), model_has_exportable_geometry);
    };
    ...
    for (auto const &opt_key : m_actions) {
        if (opt_key == "help") {
        ...
        } else if (opt_key == "repair_model") {
            // Processed transactionally after loading and before model merging.
        } else if (opt_key == "export_stl") {
            if (!m_config.opt_bool(opt_key))
                continue;
            if (!all_stl_objects_have_exportable_geometry()) {
                const std::string error_message = "--export-stl cannot export a model object with no MODEL_PART triangles.";
                BOOST_LOG_TRIVIAL(error) << error_message;
                record_exit_reson(outfile_dir, CLI_EXPORT_STL_ERROR, 0, error_message, sliced_info);
            ...
            attach_exported_model_paths(sliced_info, exported_paths, loaded_model_sources.size());
            if (!export_succeeded) {
            ...
            if (!all_stl_objects_have_exportable_geometry()) {
                const std::string error_message = "--export-stls cannot export a model object with no MODEL_PART triangles.";
                BOOST_LOG_TRIVIAL(error) << error_message;
                record_exit_reson(outfile_dir, CLI_EXPORT_STL_ERROR, 0, error_message, sliced_info);
                flush_and_exit(CLI_EXPORT_STL_ERROR);
            }
        ...
        } else if (opt_key == "export_multicolor_obj") {
            if (m_models.size() != 1) {
                const std::string error_message = "--export-multicolor-obj currently requires exactly one output model.";
                BOOST_LOG_TRIVIAL(error) << error_message;
                record_exit_reson(outfile_dir, CLI_EXPORT_OBJ_ERROR, 0, error_message, sliced_info);
                flush_and_exit(CLI_EXPORT_OBJ_ERROR);
            }
            if (!model_has_exportable_geometry(m_models.front())) {
                const std::string error_message = "--export-multicolor-obj cannot export a model with no MODEL_PART triangles.";
                BOOST_LOG_TRIVIAL(error) << error_message;
                record_exit_reson(outfile_dir, CLI_EXPORT_OBJ_ERROR, 0, error_message, sliced_info);
                flush_and_exit(CLI_EXPORT_OBJ_ERROR);
            }

            const bool annotations_were_cleared = std::any_of(sliced_info.model_repairs.begin(), sliced_info.model_repairs.end(),
                                                              [](const model_repair_info_t& repair_info) {
                                                                  return repair_info.engine == "cgal" &&
                                                                         std::any_of(repair_info.cgal_report.volume_reports.begin(),
                                                                                     repair_info.cgal_report.volume_reports.end(),
                                                                                     [](const ModelRepairVolumeReport& volume) {
                                                                                         return volume.annotations_cleared;
                                                                                     });
                                                              });
            if (annotations_were_cleared) {
                const std::string error_message =
                    "--export-multicolor-obj was refused because model repair changed topology and cleared MMU/paint annotations.";
                BOOST_LOG_TRIVIAL(error) << error_message;
                record_exit_reson(outfile_dir, CLI_EXPORT_OBJ_ERROR, 0, error_message, sliced_info);
                flush_and_exit(CLI_EXPORT_OBJ_ERROR);
            }
            ...
            if (!boost::algorithm::iends_with(export_path.extension().string(), ".obj"))
                export_path.replace_extension(".obj");
            ...
            if (colors_option) {
                for (const std::string& color : colors_option->values) {
                    ...
                    if (!Slic3r::GUI::BitmapCache::parse_color4(color, rgba)) {
                        const std::string error_message = "Invalid filament color while exporting multicolor OBJ: " + color;
                        BOOST_LOG_TRIVIAL(error) << error_message;
                        record_exit_reson(outfile_dir, CLI_EXPORT_OBJ_ERROR, 0, error_message, sliced_info);
                        flush_and_exit(CLI_EXPORT_OBJ_ERROR);
                    }
            ...
            boost::filesystem::path mtl_path = export_path;
            mtl_path.replace_extension(".mtl");
            const std::vector<std::string> output_files{export_path.string(), mtl_path.string()};
            sliced_info.obj_exports.push_back({"filament_mmu", "filament_and_mmu_assignments", false, false, false, output_files});
            attach_exported_model_paths(sliced_info, {output_files}, loaded_model_sources.size());
            ...
            if (!boost::algorithm::iends_with(export_path.extension().string(), ".obj"))
                export_path.replace_extension(".obj");
            boost::filesystem::path mtl_export_path = export_path;
            mtl_export_path.replace_extension(".mtl");
            ...
            sliced_info.output_files.push_back(export_path.string());
            sliced_info.output_files.push_back(mtl_export_path.string());
            sliced_info.obj_exports.push_back(
                {"texture_cluster", "texture_color_clusters", false, false, false, {export_path.string(), mtl_export_path.string()}});
        } else if (opt_key == "export_obj") {
            if (!m_config.opt_bool(opt_key))
                continue;
            if (!all_obj_models_have_exportable_geometry()) {
                const std::string error_message = "--export-obj cannot export a model with no MODEL_PART triangles.";
                BOOST_LOG_TRIVIAL(error) << error_message;
                record_exit_reson(outfile_dir, CLI_EXPORT_OBJ_ERROR, 0, error_message, sliced_info);
                flush_and_exit(CLI_EXPORT_OBJ_ERROR);
            }
            for (auto &model : m_models)
                model.add_default_instances();
            std::vector<std::vector<std::string>> exported_paths;
            const bool                            export_succeeded = this->export_models(IO::OBJ, std::string(), &exported_paths);
            attach_exported_model_paths(sliced_info, exported_paths, loaded_model_sources.size());
            if (!export_succeeded) {
                record_exit_reson(outfile_dir, CLI_EXPORT_OBJ_ERROR, 0, cli_errors[CLI_EXPORT_OBJ_ERROR], sliced_info);
                flush_and_exit(CLI_EXPORT_OBJ_ERROR);
            }
            for (const std::vector<std::string>& paths : exported_paths)
                sliced_info.obj_exports.push_back({"geometry_only", "triangle_mesh", false, false, false, paths});
...
*/
    ...
    if (duplicate_count > 0) {
        ...
        result_written = record_exit_reson(outfile_dir, 0, plate_to_slice, cli_errors[0], sliced_info, record_key_values);
    } else {
        result_written = record_exit_reson(outfile_dir, 0, plate_to_slice, cli_errors[0], sliced_info);
    }
    if (!result_written)
        flush_and_exit(CLI_RESULT_WRITE_ERROR);
...
bool CLI::setup(int argc, char **argv)
{
    ...
    for (auto const &opt_key : opt_order) {
        if (cli_actions_config_def.has(opt_key)) {
            const ConfigOptionDef* action_def = cli_actions_config_def.get(opt_key);
            if (action_def != nullptr && action_def->type == coBool && !m_config.opt_bool(opt_key))
                continue;
...
void CLI::print_help(bool include_print_options, PrinterTechnology printer_technology) const
{
    ...
    boost::nowide::cout << SLIC3R_APP_KEY << "-" << SoftFever_VERSION << ":" << std::endl
                        << "Usage: orca-slicer [ OPTIONS ] [ file.3mf/file.stl/file.obj ... ]" << std::endl
                        << std::endl
                        << "OPTIONS:" << std::endl;
...
std::string CLI::output_filepath(const Model &model, IO::ExportFormat format) const
{
    ...
    const std::string cmdline_param = m_config.opt_string("outputdir");
    if (!cmdline_param.empty()) {
        // --outputdir is always a directory, including when it does not exist yet.
        const boost::filesystem::path output_dir(cmdline_param);
        boost::system::error_code     error_code;
        boost::filesystem::create_directories(output_dir, error_code);
        if (error_code) {
            BOOST_LOG_TRIVIAL(error) << "Failed creating output directory " << output_dir.string() << ": " << error_code.message();
            return {};
        }
        proposed_path = output_dir / proposed_path.filename();
    }

    // Never truncate an OBJ input when the proposed output resolves to the
    // same path. Preserve the source and make the generated artifact explicit.
    if (format == IO::OBJ) {
        std::vector<boost::filesystem::path> input_paths;
        input_paths.reserve(m_input_files.size());
        for (const std::string& input_file : m_input_files)
            if (!input_file.empty())
                input_paths.emplace_back(input_file);
        for (const Model& loaded_model : m_models) {
            for (const ModelObject* object : loaded_model.objects) {
                if (object == nullptr)
                    continue;
                if (!object->input_file.empty())
                    input_paths.emplace_back(object->input_file);
                for (const ModelVolume* volume : object->volumes)
                    if (volume != nullptr && !volume->source.input_file.empty())
                        input_paths.emplace_back(volume->source.input_file);
            }
        }

        bool       path_resolution_failed = false;
        const auto conflicts_with_input   = [&input_paths, &path_resolution_failed](const boost::filesystem::path& candidate) {
            boost::system::error_code     output_error;
            const boost::filesystem::path absolute_output = boost::filesystem::absolute(candidate, output_error).lexically_normal();
            if (output_error) {
            ...
            for (const boost::filesystem::path& input_path : input_paths) {
                boost::system::error_code     input_error;
                const boost::filesystem::path absolute_input = boost::filesystem::absolute(input_path, input_error).lexically_normal();
                if (input_error)
                    continue;
#ifdef _WIN32
                const bool same_path = boost::algorithm::iequals(absolute_output.string(), absolute_input.string());
#else
                const bool same_path = absolute_output == absolute_input;
#endif
                boost::system::error_code equivalent_error;
                const bool equivalent = boost::filesystem::exists(absolute_output) && boost::filesystem::exists(absolute_input) &&
                                        boost::filesystem::equivalent(absolute_output, absolute_input, equivalent_error) &&
                                        !equivalent_error;
                if (same_path || equivalent)
                    return true;
            }
        ...
        if (conflicts_with_input(proposed_path)) {
            if (path_resolution_failed) {
                BOOST_LOG_TRIVIAL(error) << "Failed resolving OBJ output path " << proposed_path.string();
                return {};
            }
            const bool        repair_requested      = std::find(m_actions.begin(), m_actions.end(), "repair_model") != m_actions.end();
            const std::string suffix                = repair_requested ? "_repaired" : "_exported";
            const boost::filesystem::path parent    = proposed_path.parent_path();
            const std::string             stem      = proposed_path.stem().string();
            const std::string             extension = proposed_path.extension().string();
            size_t                        sequence  = 1;
            do {
                const std::string numbered_suffix = sequence == 1 ? suffix : suffix + "_" + std::to_string(sequence);
                proposed_path                     = parent / (stem + numbered_suffix + extension);
                ++sequence;
            } while (conflicts_with_input(proposed_path) && !path_resolution_failed);
            if (path_resolution_failed) {
                BOOST_LOG_TRIVIAL(error) << "Failed resolving OBJ output path " << proposed_path.string();
                return {};
            }
            BOOST_LOG_TRIVIAL(info) << "OBJ output matched an input path; exporting safely to " << proposed_path.string();
        }
    }
...
std::string CLI::output_filepath(const ModelObject &object, unsigned int index, IO::ExportFormat format, std::string path_dir) const
{
    ...
    boost::filesystem::path source_name(object.name.empty() ? object.input_file : object.name);
    source_name = source_name.filename();
    if (source_name.empty())
        source_name = "model";
    source_name.replace_extension(ext);
    file_name = "obj_" + std::to_string(index) + "_" + source_name.string();

    BOOST_LOG_TRIVIAL(trace) << __FUNCTION__ << ": dir = " << path_dir << ", file_name=" << file_name;
    if (path_dir.empty()) {
