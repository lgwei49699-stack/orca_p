...
bool PartPlate::check_outside(int obj_id, int instance_id, BoundingBoxf3* bounding_box)
{
	...
	if (instance_box.min.z() < SINKING_Z_THRESHOLD) {
		...
		if (plate_box.intersects(instance_box)) {
			...
			// CLI plate lists do not own a GUI Plater. Fall back to the plate's configured
			// height so checking a transformed or intentionally sinking model stays safe.
			double printable_height = static_cast<double>(m_height);
			if (m_plater != nullptr)
				printable_height = m_plater->build_volume().printable_height();
