                        ...
                        if (state == ConnectStatus::ConnectStatusOk) {
                            ...
                            if (!already_disconnected) {
                                ...
                                if (msg == "5") {
                                    ...
                                    text = wxString::Format(_L("Connect %s failed! [SN:%s, code=%s]"), from_u8(obj->dev_name), obj->dev_id,
                                                            msg);
                                    wxGetApp().show_dialog(text);
                                }
                            }
...
wxGLContext* GUI_App::init_glcontext(wxGLCanvas& canvas) { return m_opengl_mgr.init_glcontext(canvas); }
...
void GUI_App::init_app_config()
{
    ...
    unsigned int log_level = Slic3r::level_string_to_boost(app_config->get("log_severity_level"));
    if (log_level < 3)
        log_level = 3;
...
void GUI_App::start_sync_user_preset(bool with_progress_dlg)
{
    ...
    if (with_progress_dlg) {
        ...
        cancelFn   = [this, dlg]() { return m_is_closing || dlg->WasCanceled(); };
