...
TextureImportDialog::TextureImportDialog(
    wxWindow*                        parent,
    std::shared_ptr<const Slic3r::TexturedMesh> textured_mesh,
    ...
    , m_textured_mesh_owner(std::move(textured_mesh))
    , m_textured_mesh(*m_textured_mesh_owner)
...
void TextureImportDialog::build_params_panel(wxWindow* parent, wxSizer* sizer)
{
    ...
    m_color_spin->GetTextCtrl()->Bind(wxEVT_TEXT, &TextureImportDialog::on_color_spin_text_changed, this);
    ...
    m_smooth_spin->GetTextCtrl()->Bind(wxEVT_TEXT, &TextureImportDialog::on_smooth_spin_text_changed, this);
...
void TextureImportDialog::start_computation(bool auto_color, bool initial)
{
    ...
    const Slic3r::TexturedMesh* textured_mesh = &m_textured_mesh;
    wxEvtHandler*               handler       = this;

    m_worker = std::make_unique<std::thread>([this, settings, textured_mesh, handler]() {
        ...
        bool ok = Slic3r::texture_to_painting(*textured_mesh, result, settings, progress_cb, cancel_cb);
...
Slic3r::PaintedMesh TextureImportDialog::take_painted_mesh()
{
    return std::move(m_painted);
}
